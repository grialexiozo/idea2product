...
export interface FluxKontextMaxParams {
  prompt: string;
  image?: string;
  mask_image?: string;
  strength?: number;
 ...
 */
export const wsFluxKontextMax = dataActionWithPermission(
  "wsFluxKontextMax",
  async (
    params: FluxKontextMaxParams,
    ...
    try {
      ...
      const checkResult = await taskCallCheck(params, defaultParams, CODE.FluxKontextMax, userContext);
      ...
      const task = await TasksEdit.create({
        ...
        type: "wsFluxKontextMax",
        status: TaskStatus.PENDING,
        title: "AI Image Generation Task",
        description: "Process image generation request via WaveSpeed FluxKontextMax API",
        progress: 0,
        startedAt: new Date(),
        checkInterval: 5,
 ...
 */
export const wsFluxKontextMaxStatus = dataActionWithPermission(
  "wsFluxKontextMaxStatus",
  async (
    taskId: string,
    userContext: UserContext // Pass userContext as a parameter
  ): Promise<TaskInfo> => {
      ...
      if (taskInfo.status === "failed" || taskInfo.status === "cancelled" || taskInfo.status === "completed") {
        Promise.resolve().then(async () => {
          ...
          if (taskInfo.status === "failed") {
              const billableMetric: BillableMetric | undefined = await BillableMetricsQuery.getByCode(CODE.FluxKontextMax);
              taskCallRecordRevoke(parseInt(task?.externalMetricEventId!, 0), task?.currentRequestAmount!, CODE.FluxKontextMax, billableMetric!, userContext)
              .then(() => {
