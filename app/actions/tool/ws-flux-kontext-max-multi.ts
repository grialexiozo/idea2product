...
export interface FluxKontextMaxMultiParams {
  prompt: string;
  images?: string[];
  mask_image?: string;
  strength?: number;
 ...
 */
export const wsFluxKontextMaxMulti = dataActionWithPermission(
  "wsFluxKontextMaxMulti",
  async (
    params: FluxKontextMaxMultiParams,
    ...
    try {
      ...
      const checkResult = await taskCallCheck(params, defaultParams, CODE.FluxKontextMaxMulti, userContext);
      ...
      const task = await TasksEdit.create({
        ...
        type: "wsFluxKontextMaxMulti",
        status: TaskStatus.PENDING,
        title: "AI Image Generation Task",
        description: "Process image generation request via WaveSpeed FluxKontextMaxMulti API",
        progress: 0,
        startedAt: new Date(),
        checkInterval: 5,
 ...
 */
export const wsFluxKontextMaxMultiStatus = dataActionWithPermission(
  "wsFluxKontextMaxMultiStatus",
  async (
    taskId: string,
    userContext: UserContext // Pass userContext as a parameter
  ): Promise<TaskInfo> => {
      ...
      if (taskInfo.status === "failed" || taskInfo.status === "cancelled" || taskInfo.status === "completed") {
        Promise.resolve().then(async () => {
          ...
          if (taskInfo.status === "failed") {
              const billableMetric: BillableMetric | undefined = await BillableMetricsQuery.getByCode(CODE.FluxKontextMaxMulti);
              taskCallRecordRevoke(parseInt(task?.externalMetricEventId!, 0), task?.currentRequestAmount!, CODE.FluxKontextMaxMulti, billableMetric!, userContext)
              .then(() => {
