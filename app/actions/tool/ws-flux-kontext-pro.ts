...
export interface FluxKontextProParams {
  prompt: string;
  image?: string;
  mask_image?: string;
  strength?: number;
 ...
 */
export const wsFluxKontextPro = dataActionWithPermission(
  "wsFluxKontextPro",
  async (
    params: FluxKontextProParams,
    ...
    try {
      ...
      const checkResult = await taskCallCheck(params, defaultParams, CODE.FluxKontextPro, userContext);
      ...
      const task = await TasksEdit.create({
        ...
        type: "wsFluxKontextPro",
        status: TaskStatus.PENDING,
        title: "AI Image Generation Task",
        description: "Process image generation request via WaveSpeed FluxKontextPro API",
        progress: 0,
        startedAt: new Date(),
        checkInterval: 5,
 ...
 */
export const wsFluxKontextProStatus = dataActionWithPermission(
  "wsFluxKontextProStatus",
  async (
    taskId: string,
    userContext: UserContext // Pass userContext as a parameter
  ): Promise<TaskInfo> => {
      ...
      if (taskInfo.status === "failed" || taskInfo.status === "cancelled" || taskInfo.status === "completed") {
        Promise.resolve().then(async () => {
          ...
          if (taskInfo.status === "failed") {
              const billableMetric: BillableMetric | undefined = await BillableMetricsQuery.getByCode(CODE.FluxKontextPro);
            taskCallRecordRevoke(parseInt(task?.externalMetricEventId!, 0), task?.currentRequestAmount!, CODE.FluxKontextPro, billableMetric!, userContext)
              .then(() => {
