...
import { FluxKontextProMultiRequest } from "@/sdk/wavespeed/requests/flux-kontext-pro-multi.request";
...
export interface FluxKontextProMultiParams {
  prompt: string;
  images?: string[];
  mask_image?: string;
  strength?: number;
 ...
 */
export const wsFluxKontextProMulti = dataActionWithPermission(
  "wsFluxKontextProMulti",
  async (
    params: FluxKontextProMultiParams,
    ...
    try {
      ...
      const checkResult = await taskCallCheck(params, defaultParams, CODE.FluxKontextProMulti, userContext);
      ...
      const task = await TasksEdit.create({
        ...
        type: "wsFluxKontextProMulti",
        status: TaskStatus.PENDING,
        title: "AI Image Generation Task",
        description: "Process image generation request via WaveSpeed FluxKontextProMulti API",
        progress: 0,
        startedAt: new Date(),
        checkInterval: 5,
 ...
 */
export const wsFluxKontextProMultiStatus = dataActionWithPermission(
  "wsFluxKontextProMultiStatus",
  async (
    taskId: string,
    userContext: UserContext // Pass userContext as a parameter
  ): Promise<TaskInfo> => {
      ...
      if (taskInfo.status === "failed" || taskInfo.status === "cancelled" || taskInfo.status === "completed") {
        Promise.resolve().then(async () => {
          ...
          if (taskInfo.status === "failed") {
              const billableMetric: BillableMetric | undefined = await BillableMetricsQuery.getByCode(CODE.FluxKontextProMulti);
            taskCallRecordRevoke(parseInt(task?.externalMetricEventId!, 0), task?.currentRequestAmount!, CODE.FluxKontextProMulti, billableMetric!, userContext)
              .then(() => {
